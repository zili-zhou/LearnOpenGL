...
unsigned int LoadCubeMap(const std::vector<std::string>& skyboxFaces);
...
int main()
{
	...
	glm::vec3 glasscubePositions[] = {
	  glm::vec3(1.8f, 2.0f, 12.3f),
	  glm::vec3(-2.4f, 2.4f, 1.5f),
	  glm::vec3(1.7f,  5.0f, 9.5f),
	  glm::vec3(-3.3f, 2.0f, 5.5f),
	  glm::vec3(-1.5f,  5.0f, 8.1f),
	  glm::vec3(2.5f,  0.2f, 5.5f),
	  glm::vec3(-3.3f,  4.5f, 7.5f)
	};
	...
	float skyboxVertives[] =
	{
		// positions  front render inner face        
		-1.0f,  1.0f, -1.0f,
		-1.0f, -1.0f, -1.0f,
		 1.0f, -1.0f, -1.0f,
		 1.0f, -1.0f, -1.0f,
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(skyboxVertives), &skyboxVertives, GL_STATIC_DRAW);
	...
	//skybox shader
	Shader SkyboxShader("..//Shaders//cubemapvs.vert", "..//Shaders//cubemapfs.frag");
	SkyboxShader.use();
	SkyboxShader.SetInt("skybox", 4);

	//glass cube shader
	Shader GlassCubeShader("..//Shaders//transparentCubevs.vert", "..//Shaders//transparentCubefs.frag");
	GlassCubeShader.use();
	GlassCubeShader.SetInt("skybox", 4);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < sizeof(glasscubePositions) / sizeof(glm::vec3); i++)
		{
			...
			glActiveTexture(GL_TEXTURE4);
			glBindTexture(GL_TEXTURE_CUBE_MAP, skyboxTexture);
			model = glm::translate(glm::mat4(1.0f), glasscubePositions[i]);
			//float angle = 10.0f * i;
			//model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
...
unsigned int LoadCubeMap(const std::vector<std::string>& skyboxFaces)
{
	...
	{
		unsigned char* data = stbi_load(skyboxFaces[i].c_str(), &Width, &Height, &ColorChannel, 0);
		GLenum format;
		if (data != nullptr)
		{
